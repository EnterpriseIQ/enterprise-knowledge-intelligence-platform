...
def write_pdf(filename: str, title: str, paragraphs: list[str]) -> Path:
    ...
    if _HAVE_REPORTLAB:
        doc = SimpleDocTemplate(
            str(path), pagesize=LETTER, topMargin=0.8 * inch, bottomMargin=0.8 * inch
        )
...
def gen_hr():
    p = write_pdf(
        ...
        "Acme Corp Employee Handbook (2025)",
        [
            "## Purpose",
    ...
    p = write_pdf(
        "hr_hiring_guidelines.pdf",
        "Hiring and Recruitment Guidelines (Confidential)",
        [
            "## Scope",
...
def gen_engineering():
    p = write_pdf(
        "eng_architecture_standards.pdf",
    ...
    p = write_pdf(
        "eng_deployment_guide.pdf",
        "Production Deployment Guide and Standards",
        [
            "## Deployment Pipeline",
    ...
    p = write_pdf(
        "eng_incident_reports.pdf",
        "Platform Incident Reports (Confidential)",
        [
    ...
    p = write_pdf(
        "eng_platform_documentation.pdf",
...
def gen_finance():
    p = write_pdf(
        "fin_budget_report_2025.pdf",
        "Annual Budget Report and Allocations 2025 (Confidential)",
        [
            "## Executive Summary",
    ...
    p = write_pdf(
        "fin_expense_report_q1.pdf",
        "Q1 2025 Expense Report (Confidential)",
        [
            "## Summary",
    ...
    p = write_pdf(
        "fin_department_forecast.pdf",
        "Department Forecast 2025-2026 (Confidential)",
        [
            "## Methodology",
...
def gen_compliance():
    ...
    p = write_pdf(
        "comp_audit_report_2025.pdf",
        "Internal Audit Report 2025 (Restricted)",
        [
            "## Scope",
    ...
    p = write_pdf(
        "comp_regulatory_findings.pdf",
        "Regulatory Findings and Remediation Log (Restricted)",
        [
            "## Regulatory Context",
...
def gen_operations():
    ...
    cur.executemany(
        ...
        [
            (
                ...
                "search",
                "resolved",
                "Elevated search latency after index rebuild",
            ),
            (
                "OPS-5031",
                "2025-05-18",
                ...
                "auth",
                "resolved",
                "Intermittent 401s due to signing key cache",
            ),
            (
                "OPS-5044",
                "2025-05-22",
                ...
                "database",
                "open",
                "db-eu-02 replica lag exceeding threshold",
            ),
            (
                "OPS-5050",
                "2025-05-25",
                ...
                "api-gateway",
                "open",
                "Connection pool warnings under peak load",
            ),
        ],
    )
    conn.commit()
    ...
    _register(
        "ops-sql",
        "Operations Database (SQL: servers, tickets)",
        db_path,
        "sql",
        "Operations",
        "confidential",
        ...
        for i, (sev, cat, desc) in enumerate(
            [
                (
                    "low",
                    "auth",
                    "5 failed login attempts for user fin_carol from a new device, then success with MFA.",
                ),
                (
                    "medium",
                    "access",
                    "User eng_dave attempted to access a Finance document and was denied by RBAC.",
                ),
                (
                    "high",
                    "exfiltration",
                    "Anomalous bulk export attempt blocked: 4,200 records requested outside business hours.",
                ),
                (
                    "low",
                    "config",
                    "TLS certificate for api-gateway renewed automatically 14 days before expiry.",
                ),
            ]
        )
    ]
