...
# python-dotenv ships with uvicorn[standard]; if absent we silently rely on
...
# Chunking
# --------------------------------------------------------------------------- #
CHUNK_SIZE = int(getenv("ERAG_CHUNK_SIZE", "900"))  # characters
CHUNK_OVERLAP = int(getenv("ERAG_CHUNK_OVERLAP", "150"))  # characters
...
EMBEDDING_MODEL = getenv("ERAG_EMBEDDING_MODEL", "all-MiniLM-L6-v2")
EMBEDDING_DIM_FALLBACK = int(getenv("ERAG_EMBEDDING_DIM", "384"))

# --------------------------------------------------------------------------- #
# Retrieval
# --------------------------------------------------------------------------- #
TOP_K = int(getenv("ERAG_TOP_K", "5"))
CANDIDATE_K = int(getenv("ERAG_CANDIDATE_K", "20"))
# Weight given to dense (semantic) vs. sparse (BM25) scores during fusion.
HYBRID_ALPHA = float(getenv("ERAG_HYBRID_ALPHA", "0.6"))

# Confidence thresholds used to label answers for the end user.
CONFIDENCE_HIGH = float(getenv("ERAG_CONF_HIGH", "0.55"))
CONFIDENCE_LOW = float(getenv("ERAG_CONF_LOW", "0.30"))
